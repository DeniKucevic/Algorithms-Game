const getUnvisitedNeighbors = (grid, current, finish) => {
	...
	for (let i = 0; i < unvisitedNeighbors.length; i += 1) {
		const distance = Math.abs(current.col - finish.col) + Math.abs(current.row - finish.row);
		if (unvisitedNeighbors[i].distance > distance) {
			unvisitedNeighbors[i].distance = distance;
		}
		unvisitedNeighbors[i].parentNode = current;
	}
	unvisitedNeighbors.sort((nodeA, nodeB) => nodeA.distance - nodeB.distance);
...
const aStar = (stage) => {
    ...
    if (current.isWall) continue; // skip a wall
		...
		getUnvisitedNeighbors(grid, current, finish); // loop if we did not find it
	}
	return visitedNodes;
};

const aStarPath = (finish) => {
	const path = [];
	let current = finish;
	while (current !== null) {
